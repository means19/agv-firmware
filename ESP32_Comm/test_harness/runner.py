...
def _run_case_3(client: HarnessMqttClient, config: HarnessConfig, dry_run: bool) -> TestOutcome:
    """Interactive physical test for node traversal and kinematics calculation."""
    if dry_run:
        return TestOutcome(name="TC3", passed=True, details="Skipped dry-run for physical interactive test.")

    order_topic = config.order_topic
    client.subscribe([config.state_topic])

    # 1. Gửi bản tin điều hướng ngã tư
    turn_order = build_turn_order_payload(config)
    client.publish_json(order_topic, turn_order, qos=1)
    
    print("\n[TC3] Đã gửi Order rẽ trái. Lộ trình: node_start (0,0) -> node_intersect (1,0) -> node_end (1,1)")
    print("      Lưu ý: ESP32 có 60 giây để chờ bạn quét từng thẻ RFID.")

    # 2. Chờ quét thẻ thứ nhất
    print("\n>>> HÀNH ĐỘNG: Hãy quét thẻ RFID được map với 'node_start'...")
    state_start = client.wait_for(
        config.state_topic, 
        predicate=lambda p: p.get("lastNodeId") == "node_start", 
        timeout_s=60.0
    )
    if not state_start: return TestOutcome(name="TC3", passed=False, details="Timeout khi chờ quét thẻ node_start.")
    print("    [+] ESP32 xác nhận đã đến node_start. AGV đang tiến về ngã tư.")

    # 3. Chờ quét thẻ thứ hai (Ngã tư)
    print("\n>>> HÀNH ĐỘNG: Hãy quét thẻ RFID được map với 'node_intersect'...")
    state_intersect = client.wait_for(
        config.state_topic, 
        predicate=lambda p: p.get("lastNodeId") == "node_intersect", 
        timeout_s=60.0
    )
    if not state_intersect: return TestOutcome(name="TC3", passed=False, details="Timeout khi chờ quét thẻ node_intersect.")
    print("    [+] ESP32 xác nhận đã đến ngã tư (node_intersect). Tại lúc này, STM32 phải đang nhận lệnh RẼ TRÁI (0x01).")

    # 4. Chờ quét thẻ thứ ba (Đích)
    print("\n>>> HÀNH ĐỘNG: Hãy quét thẻ RFID được map với 'node_end'...")
    state_end = client.wait_for(
        config.state_topic, 
        predicate=lambda p: p.get("lastNodeId") == "node_end", 
        timeout_s=60.0
    )
    if not state_end: return TestOutcome(name="TC3", passed=False, details="Timeout khi chờ quét thẻ node_end.")
    print("    [+] ESP32 xác nhận đã đến đích (node_end). Lộ trình hoàn tất.")

    return TestOutcome(name="TC3", passed=True, details="Xe đã hoàn thành di chuyển qua 3 mốc tọa độ vật lý thành công!")

def build_parser() -> argparse.ArgumentParser:
    """Create the CLI parser."""

    parser = argparse.ArgumentParser(description="AGV system integration harness")
    parser.add_argument("--case", choices=["tc1", "tc2", "tc3", "all"], default="all")
...
def run() -> int:
    ...
    try:
        ...
        if args.case in ("tc3", "all"):
            outcomes.append(_run_case_3(client, config, args.dry_run))
    finally:
        if not args.dry_run:
            client.close()
